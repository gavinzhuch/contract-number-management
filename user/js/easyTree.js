...
(function ($) {
    $.fn.EasyTree = function (options) {
        ...
        //this var maybe change
        var task_order_depth = 13;
        var defaults = {
            selectable: true,
            deletable: false,
            editable: false,
            addable: false,
            i18n: {
                deleteNull: 'Please select the item to be deleted.',
                deleteConfirmation: 'Are you sure you want to perform the delete operation? All data will be deleted.',
                confirmButtonLabel: 'Confirm',
                editNull: 'Please select the item you want to edit.',
                editMultiple: 'Only one item can be edited at a time',
                addMultiple: 'Only one item can be added at a time',
                collapseTip: 'collapse',
                expandTip: 'expand',
                selectTip: 'Select',
                unselectTip: 'Not selected',
                editTip: 'Edit',
                addTip: 'Add',
                deleteTip: 'Delete',
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        if (selected.length <= 0) {
                            //add contract_numbers
                            $(easyTree).find(' > ul').append($(item));
                            // console.log($(createInput).find('input').val());
                            ...
                            $.ajax({
            						        ...
            						        dataType: "json",
            						        data: {
            						            value: $(createInput).find('input').val(),
                                    parent_id: 0,
                                    level:1,
                                    status:1
            						        }
                        ...
                        if (options.selectable) {
                            ...
                            $(item).find(' > span > a').click(function (e) {
                                ...
                                if (options.deletable || options.editable || options.addable) {
                                    ...
                                    if (options.addable) {
                                        //task_order_number  node can not have children nodes
                                        //task_order_depth maybe change accounting to the html strcture
                                        if ($(easyTree).find('li.li_selected').parents().length >= task_order_depth)  {
                ...
                $(easyTree).find('li > span > a').click(function (e) {
                    ...
                    if (options.deletable || options.editable || options.addable) {
                        ...
                        if (options.addable) {
                            ...
                            if ($(easyTree).find('li.li_selected').parents().length >= task_order_depth)  {
                                $(easyTree).find('.easy-tree-toolbar .create > button').addClass('disabled');
                                // $(easyTree).find('.easy-tree-toolbar  .input-group> button').addClass('disabled');
                                if ($(createInput).length) {
                                    $(easyTree).find('.create .input-group .input-group-btn > button').addClass('disabled');
                                }
                            }
                            else {
                              $(easyTree).find('.easy-tree-toolbar .create > button').removeClass('disabled');
                              if ($(createInput).length) {
                                  $(easyTree).find('.create .input-group .input-group-btn > button').removeClass('disabled');
                              }
                            }
                        }
